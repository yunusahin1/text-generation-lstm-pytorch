...
def preprocess_data(df: pd.DataFrame) -> Dict[str, int]:
    ...
    df = df.rename(columns={'abstract': 'text'})[:500]
...
def create_sequences(all_text: str, char_to_idx: Dict[str, int]) -> (List[List[int]], List[int]):
    seq_length = 100
    step_size = 1
    X = []
    y = []
