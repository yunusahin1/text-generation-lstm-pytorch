...
def generate_text(model, start_text, char_to_idx, idx_to_char, length=500, temperature=1.0, device='cpu'):
    ...
    with torch.no_grad():
        for _ in range(length):
            x = torch.zeros(1, len(current_seq), vocab_size).to(device)
            for i, char_idx in enumerate(current_seq):
                x[0, i, char_idx] = 1.0
